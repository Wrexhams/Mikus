...
atrtrailer = nil
-- dgun combo
local item_list = {u8'Êóëàê', u8'Êàñòåò', u8'Êëþøêà äëÿ ãîëüôà', u8'Ïîëèöåéñêàÿ äóáèíêà', u8'Íîæ', u8'Áåéñáîëüíàÿ áèòà', u8'Ëîïàòà', u8'Êèé', u8'Êàòàíà', u8'Áåíçîïèëà', u8'Äâóõñòîðîííèé äèëäî', u8'Äèëäî', u8'Âèáðàòîð', u8'Ñåðåáðÿíûé âèáðàòîð', u8'Áóêåò öâåòîâ', u8'Òðîñòü', u8'Ãðàíàòà', u8'Ñëåçîòî÷èâûé ãàç', u8'Êîêòåéëü Ìîëîòîâà', u8' ', u8' ', u8' ', u8'Ïèñòîëåò', u8'USP-S (Òàçåð)', u8'Desert Eagle', u8'Äðîáîâèê', u8'Îáðåç', u8'Ñêîðîñòðåëüíûé äðîáîâèê', u8'UZI', u8'MP5', u8'Êàëàø', u8'Ì4', u8'TEC-9', u8'Îõîòíè÷üå ðóæüå', u8'Ñíàéïåðêà', u8'ÐÏÃ', u8'Ñàìîíàâîäÿùååñÿ ÐÏÃ', u8'Îãíåìåò', u8'Ìèíèãàí', u8'Ñóìêà ñ òðîòèëîì', u8'Äåòîíàòîð', u8'Áàëëîí÷èê', u8'Îãíåòóøèòåëü', u8'Ôîòîàïïàðàò', u8'Î÷êè íî÷íîãî âèäåíèÿ', u8'Òåïëîâèçîð', u8'Ïàðàøþò'}
...
imgui.OnFrame(function() return found_update[0] end, function()
    ...
    imgui.CenterText('(credits: @arz_run, @NN, @NN, @NN)')
    imgui.PopFont()
    imgui.CenterText('')
    imgui.PushFont(upd1)
    ...
    imgui.CenterText('')
    imgui.CenterText('')
    if imgui.Button(fa.DOWNLOAD..u8' ÎÁÍÎÂÈÒÜ', imgui.ImVec2(650, 40)) then
        updateScript(lmUrl, lmPath)
    end
    if imgui.Button(fa.FORWARD..u8' ÏÐÎÏÓÑÒÈÒÜ', imgui.ImVec2(650, 40)) then
       found_update[0] = not found_update[0]
       notf4(u8'Îáíîâëåíèå ñêðèïòà ïðîïóùåíî')
    end
    imgui.End()
end)
-- demo window
...
imgui.OnFrame(function() return settings.menu.sendalt[0] end, function()
    ...
    if imgui.Button(fa.HANDSHAKE_ANGLE..u8' Âçàèìîäåéñòâèå', imgui.ImVec2(settings.menu.sendaltwidth[0], settings.menu.sendaltheight[0])) then
        lua_thread.create(function()
            local bs = raknetNewBitStream()
...
imgui.OnFrame(function() return menusettings[0] end, function()
    imgui.Begin(u8'Íàñòðîéêè ìåíþ', menusettings, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoTitleBar)
    imgui.CenterText(fa.FOLDER..u8' Îáùèå íàñòðîéêè ìåíþ')
    imgui.Separator()
    if imgui.SliderFloat(fa.MAXIMIZE..u8' Ðàçìåð øðèôòà', settings.menu.window_scale, 1 / MONET_DPI_SCALE, 3.0) then
    ...
    imgui.Separator()
    imgui.CenterText(fa.HANDSHAKE_ANGLE..u8' Âçàèìîäåéñòâèå')
    imgui.Separator()
    if imgui.SliderInt(fa.TEXT_WIDTH..u8' Øèðèíà âçàèìîäåéñòâèÿ', settings.menu.sendaltwidth, 1, 400) then
        if settings.cfg.autosave[0] then
            ini.menu.sendaltwidth = settings.menu.sendaltwidth[0]
            save()
        end
    end
    if imgui.SliderInt(fa.TEXT_HEIGHT..u8' Âûñîòà âçàèìîäåéñòâèÿ', settings.menu.sendaltheight, 1, 400) then
        if settings.cfg.autosave[0] then
...
imgui.OnFrame(function() return window_state[0] end, function()
    ...
    if imgui.BeginChild("Tabs##"..tab, imgui.ImVec2(153 * MONET_DPI_SCALE, 405 * MONET_DPI_SCALE), false) then
        if imgui.Button(fa.FIRE..u8"  Îñíîâíîå", imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 1
            activetab = 1
        end
        if activetab == 1 then
        ...
        if imgui.Button(fa.USER..u8'  Ïåðñîíàæ', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 2
            activetab = 2
        end
        if activetab == 2 then
        ...
        if imgui.Button(fa.CAR..u8'  Ìàøèíà', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 3
            activetab = 3
        end
        if activetab == 3 then
        ...
        if imgui.Button(fa.EYE..u8'  Ïîäñâåòêà', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 4
            activetab = 4
        end
        if activetab == 4 then
        ...
        if imgui.Button(fa.GAVEL..u8'  ÒÑÐ Õåëïåð', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 5
            activetab = 5
        end
        if activetab == 5 then
        ...
        if imgui.Button(fa.SQUARE_CHECK..u8'  Êíîïî÷êè', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 6
            activetab = 6
        end
        if activetab == 6 then
        ...
        if imgui.Button(fa.GEARS..u8'  Íàñòðîéêè', imgui.ImVec2(150 * MONET_DPI_SCALE, 53 * MONET_DPI_SCALE)) then
            tab = 7
            activetab = 7
        end
        if activetab == 7 then
    ...
    if imgui.BeginChild("##MenuT", imgui.ImVec2(652 * MONET_DPI_SCALE, 458 * MONET_DPI_SCALE), false) then
        if tab == 1 then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button(u8"Îñíîâíîå") then
                subtab_1 = 1
            end
            imgui.SameLine()
            if imgui.Button(u8"Ðåíäåð") then
                subtab_1 = 2
            end
            imgui.SameLine()
            if imgui.Button(u8"Îáúåêòû") then
                subtab_1 = 3
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_1 == 1 then
                imgui.Text(u8'Àèðáðåéê àêòèâèðóåòñÿ äàáëòàïîì íà èêîíêó îðóæèÿ')
                if imgui.ToggleButton(u8'Âèäæåò àèðáðåéêà', settings.main.airbrakewidget) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton(u8'Àíòè-ìàñêà', settings.main.antimask) then
                    if settings.cfg.autosave[0] then
                        ini.main.antimask = settings.main.antimask[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Àíòè-êàï÷à', settings.main.autocaptcha) then
                    if settings.cfg.autosave[0] then
                        ini.main.autocaptcha = settings.main.autocaptcha[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Íàïîìèíàíèå î íèçêîì çàðÿäå', settings.battery.notifyLowCharge) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.battery.notifyLowCharge[0] then
                    ...
                    if imgui.SliderInt(u8'Óðîâåíü çàðÿäà', settings.battery.lowBatteryLevel, 1, 100) then
                ...
                if imgui.ToggleButton(u8"Àíòèôðèç", settings.main.antiFreeze) then
                    if settings.cfg.autosave[0] then
                        ini.main.antiFreeze = settings.main.antiFreeze[0]
                        save()
                    end
                end
                if imgui.Button(u8'Ðåêîííåêò', imgui.ImVec2(80 * MONET_DPI_SCALE, 30 * MONET_DPI_SCALE)) and not Reconnect.reconnecting and not Reconnect.waiting then
                    Reconnect.activate()
                end
                imgui.SameLine()
                if Reconnect.waiting then
                    imgui.SameLine()
                    imgui.Text(string.format(u8'Ðåêîííåêò ÷åðåç %.2f ñåêóíä...', Reconnect.remaining / 1000))
                    imgui.SameLine()
                    if imgui.Button(fa.XMARK..u8' Îòìåíà') then
                        Reconnect.abort = true
                    end
                end
                imgui.SetNextItemWidth(imgui.GetFontSize() * 15)
                imgui.SliderFloat(u8'Çàäåðæêà ðåêîííåêòà (â ñåêóíäàõ)', Reconnect.delay, 0.0, 30.0)
                ...
                imgui.PopItemWidth()
                imgui.SameLine()
                if imgui.ToggleButton(u8'Áëîêèðîâàòü ïîãîäó ñåðâåðà', settings.main.weatherblockserv) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.PopItemWidth()
                imgui.SameLine()
                if imgui.ToggleButton(u8'Áëîêèðîâàòü âðåìÿ ñåðâåðà', settings.main.timeblockserv) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_1 == 2 then
                if imgui.ToggleButton(u8'Ðóäà', settings.render.ruda) then
                    if settings.cfg.autosave[0] then
                        ini.render.ruda = settings.render.ruda[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Îëåíè', settings.render.deer) then
                    if settings.cfg.autosave[0] then
                        ini.render.deer = settings.render.deer[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Ñåìåíà íàðêî', settings.render.semena) then
                    if settings.cfg.autosave[0] then
                        ini.render.semena = settings.render.semena[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Çàêëàäêè', settings.render.narkotiki) then
                    if settings.cfg.autosave[0] then
                        ini.render.narkotiki = settings.render.narkotiki[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Äåðåâî âûñøåãî êà÷åñòâà', settings.render.derevovishkac) then
                    if settings.cfg.autosave[0] then
                        ini.render.derevovishkac = settings.render.derevovishkac[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Ïîäàðîê', settings.render.podarok) then
                    if settings.cfg.autosave[0] then
                        ini.render.podarok = settings.render.podarok[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Êëàä', settings.render.kladrender) then
                    if settings.cfg.autosave[0] then
                        ini.render.kladrender = settings.render.kladrender[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'ßáëî÷íîå äåðåâî', settings.render.yabloki) then
                    if settings.cfg.autosave[0] then
                        ini.render.yabloki = settings.render.yabloki[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Cëèâîâîå äåðåâî', settings.render.slivu) then
                    if settings.cfg.autosave[0] then
                        ini.render.slivu = settings.render.slivu[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Êîêîñíîå äåðåâî', settings.render.kokosi) then
                    if settings.cfg.autosave[0] then
                        ini.render.kokosi = settings.render.kokosi[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Ãðàôôèòè', settings.render.graffiti) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_1 == 3 then
                imgui.Text(u8'Óäàëèòü:')
                if imgui.Button(u8'Âîðîòà àðìèè ËÑ', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                ...
                imgui.SameLine()
                if imgui.ToggleButton(u8'Àâòîóäàëåíèå', settings.objects.autormlsa) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormlsa = settings.objects.autormlsa[0]
                        save()
                    end
                end
                if imgui.Button(u8'Âîðîòà àðìèè ÑÔ', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 988 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton(u8'Àâòîóäaëåíèå', settings.objects.autormsfa) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormsfa = settings.objects.autormsfa[0]
                        save()
                    end
                end
                if imgui.Button(u8'Øëàãáàóìû (ÊÏÏ)', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 968 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton(u8'Aâòîóäàëåíèå', settings.objects.autormblockpost) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormblockpost = settings.objects.autormblockpost[0]
                        save()
                    end
                end
                if imgui.Button(u8'Ôîíàðíûå ñòîëáû', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 1297 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton(u8'Àâòoóäàëåíèå', settings.objects.autormlampposts) then
                    if settings.cfg.autosave[0] then
                        ini.objects.autormlampposts = settings.objects.autormlampposts[0]
                        save()
                    end
                end
                if imgui.Button(u8'Äîðîæíûå ðåìîíòû', imgui.ImVec2(130 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                        if modeid == 1422 then
                            deleteObject(obj)
                ...
                imgui.SameLine()
                if imgui.ToggleButton(u8'Àâòoóäàëåíèå##roadrem', settings.objects.autormroadrem) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.menu.showinfo[0] then
                    imgui.Text(u8'| (Áóäåò ïîïîëíÿòüñÿ â çàâèñèìîñòè)')
                    imgui.Text(u8'| (îò âàøèõ ïîæåëàíèé â ÷àòå Miku Project)')
                end
            end
        elseif tab == 2 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.16, 0.16, 0.18, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button(u8"Ïåðñîíàæ") then
                subtab_2 = 1
            end
            imgui.SameLine()
            ...
            imgui.SameLine()
            if imgui.Button(u8"Îðóæèå") then
                subtab_2 = 3
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_2 == 1 then
                if imgui.ToggleButton(u8'Áåññìåðòèå', settings.ped.godmode_enabled) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton(u8'Âàíøîò áîòîâ', settings.ped.killbots1hit) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##scrollsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup(u8'Íàñòðîéêè àâòîñêðîëëà')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal(u8'Íàñòðîéêè àâòîñêðîëëà', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt(u8'Ïàòðîíû##as', settings.ped.pt, 1, 100) then
                        if settings.cfg.autosave[0] then
                            ini.ped.pt = settings.ped.pt[0]
                            save()
                        end
                    end
                    if imgui.SliderInt(u8'Çàäåðæêà ñêðîëëà', settings.ped.wait, 1, 1000) then
                        if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton(u8'Ôåéê ñêèëëû', settings.ped.setskills) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(400 * MONET_DPI_SCALE)
                if imgui.ToggleButton(u8'Àíòè ïàäåíèå', settings.ped.nofall) then
                    if settings.cfg.autosave[0] then
                        ini.ped.nofall = settings.ped.nofall[0]
                        save()
                    end
                end
                imgui.ToggleButton(u8'Ôëóä âçàèìîäåéñòâèåì', floodalt)
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton(u8'Àâòî +C', settings.ped.autoplusc) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(400 * MONET_DPI_SCALE)
                if imgui.ToggleButton(u8'Ïîëå çðåíèÿ', settings.ped.changefov) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##fovsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup(u8'Íàñòðîéêè ïîëÿ çðåíèÿ')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal(u8'Íàñòðîéêè ïîëÿ çðåíèÿ', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt(u8"Çíà÷åíèå FOV'a", settings.ped.fovvalue, 50, 130) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton(u8'Âèäæåò ñáèâà', settings.ped.sbiv) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton(u8'Áåñêîíå÷íûé áåã', settings.ped.infiniterun) then
                    if settings.cfg.autosave[0] then
                        ini.ped.infiniterun = settings.ped.infiniterun[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Ñêîðîñòü àíèìàöèé', settings.ped.animspeed) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##speedsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup(u8'Íàñòðîéêè ñêîðîñòè àíèìàöèé')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal(u8'Íàñòðîéêè ñêîðîñòè àíèìàöèé', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt(u8'Ìíîæèòåëü ñêîðîñòè##animspeed', settings.ped.speedint, 1, 10) then
                        if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                imgui.SetCursorPosX(200 * MONET_DPI_SCALE)
                if imgui.ToggleButton(u8'Ðàïèä', settings.ped.rapidfire) then
                    if settings.cfg.autosave[0] then
                ...
                if imgui.Button(fa.GEARS..'##rapidsettings', imgui.ImVec2(14.5 * MONET_DPI_SCALE, 14.5 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup(u8'Íàñòðîéêè ðàïèäà')
                end
                imgui.PopStyleColor(3)
                if imgui.BeginPopupModal(u8'Íàñòðîéêè ðàïèäà', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt(u8'Ìíîæèòåëü ðàïèäà##rapidfire', settings.ped.rapidint, 1, 10) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.Button(u8'Óñòàíîâèòü ñêèí', imgui.ImVec2(300 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    setskin_activate()
                end
                imgui.SameLine()
                imgui.SetNextItemWidth(imgui.GetFontSize() * 7)
                if imgui.InputInt(u8'ID Ñêèíà', settings.ped.skinid, 1, 50) then
                    if settings.cfg.autosave[0] then
                        ini.ped.skinid = settings.ped.skinid[0]
                        save()
                    end
                end
                if imgui.Button(u8'Âêëþ÷èòü áåã CJ', imgui.ImVec2(146 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    setAnimGroupForChar(PLAYER_PED, "PLAYER")
                    notf4(u8'Áåã CJ âêëþ÷åí!')
                end
                imgui.SameLine()
                if imgui.Button(u8'Âûêëþ÷èòü áåã CJ', imgui.ImVec2(146 * MONET_DPI_SCALE, 25 * MONET_DPI_SCALE)) then
                    setAnimGroupForChar(PLAYER_PED, usePlayerAnimGroup and "PLAYER" or isCharMale(PLAYER_PED) and "MAN" or "WOMAN")
                    notf4(u8'Áåã CJ âûêëþ÷åí!')
                end
                imgui.PopItemWidth()
            elseif subtab_2 == 2 then
                ...
                if imgui.ToggleButton(u8'Èãíîðèðîâàòü àâòîìîáèëè', settings.silent.ignoreCars) then
                    if settings.cfg.autosave[0] then
                        ini.silent.ignoreCars = settings.silent.ignoreCars[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Èãíîðèðîâàòü îáúåêòû', settings.silent.ignoreObj) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.Separator()
                if imgui.ToggleButton(u8'Àâòîìàòè÷åñêè âûáèðàòü äèñòàíöèþ ó îðóæèå', settings.silent.useWeaponDistance) then
                    if settings.cfg.autosave[0] then
                ...
                if not settings.silent.useWeaponDistance[0] then
                    if imgui.SliderInt(u8'Äèñòàíöèÿ ïîèñêà öåëè', settings.silent.distance, 1, 1000) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.SliderInt(u8'Ðàäèóñ ïîèñêà öåëè', settings.silent.radius, 1, 100) then
                    if settings.cfg.autosave[0] then
                        ini.silent.radius = settings.silent.radius[0]
                        save()
                    end
                end
                if imgui.SliderFloat(u8'Ïîëîæåíèå FOV ïî X', settings.silent.offsetx, -200.0, 200.0) then
                    if settings.cfg.autosave[0] then
                        ini.silent.offsetx = settings.silent.offsetx[0]
                        save()
                    end
                end
                if imgui.SliderFloat(u8'Ïîëîæåíèå FOV ïî Y', settings.silent.offsety, -200.0, 200.0) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.Separator()
                if imgui.ToggleButton(u8'Îòîáðàæàòü ëèíèþ ê öåëè', settings.silent.line) then
                    if settings.cfg.autosave[0] then
                        ini.silent.line = settings.silent.line[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Îòîáðàæàòü ïðèöåë', settings.silent.circle) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.silent.circle[0] then
                    if imgui.ToggleButton(u8'Îòîáðàæàòü áîëåå ïðîèçâîäèòåëüíûé ïðèöåë (áîëåå óäîáíûé)', settings.silent.fpscircle) then
                        if settings.cfg.autosave[0] then
                ...
                if imgui.ToggleButton(u8'Ïèñàòü ñíèçó î íàíåñåíèå óðîíà', settings.silent.printString) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.Separator()
                if imgui.ToggleButton(u8'Ïðîìàõè', settings.silent.misses) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.silent.misses[0] then
                    imgui.PushItemWidth(200)
                    if imgui.SliderInt(u8'Øàíñû íà ïðîìàõ', settings.silent.miss_ratio, 1, 100) then
                        if settings.cfg.autosave[0] then
                ...
                imgui.ToggleButton(u8'Èçìåíåíèå êàìåðû', fakemode)
            elseif subtab_2 == 3 then
                if imgui.Button(u8'Âûäàòü ãàí') then
                    givePlayerGun(settings.dgun.gunsList[0], settings.dgun.ammo[0])
                end
                imgui.PushItemWidth(350)
                if imgui.Combo(u8'Ãàíû',settings.dgun.gunsList,ImItems, #item_list) then
                    ini.dgun.gunsList = settings.dgun.gunsList[0]
                    save()
                end
                imgui.PopItemWidth()
                imgui.PushItemWidth(350)
                if imgui.SliderInt(u8'Ïàòðîíû##dgun', settings.dgun.ammo, 1, 2000) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.PopItemWidth()
                if settings.menu.showinfo[0] then
                    imgui.Text(u8"| Êèêàåò áåç Resync'a, ïîòîìó ïîäîéäåò òîëüêî äëÿ íóáîðï")
                end
            end
        elseif tab == 3 then
            imgui.PushStyleColor(imgui.Col.Button, imgui.ImVec4(0.16, 0.16, 0.18, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button(u8"Ìàøèíà") then
                subtab_3 = 1
            end
            imgui.SameLine()
            ...
            imgui.Separator()
            if subtab_3 == 1 then
                -- ===== ÈÇÌÅÍ¨ÍÍÛÉ ÁËÎÊ ÁÅÑÑÌÅÐÒÈß ÌÀØÈÍÛ =====
                if imgui.ToggleButton(u8'Áåññìåðòèå', settings.car.godmode2_enabled) then
                    if settings.cfg.autosave[0] then
                ...
                -- ==============================================
                if imgui.ToggleButton(u8'Ïîë¸ò', settings.car.flycar) then
                    if settings.cfg.autosave[0] then
                        ini.car.flycar = settings.car.flycar[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Àíòè-ïàäåíèå ñ Ìîòî', settings.car.nobike) then
                    if settings.cfg.autosave[0] then
                        ini.car.nobike = settings.car.nobike[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Áåñêîíå÷íîå òîïëèâî', settings.car.infinitefuel) then
                    if settings.cfg.autosave[0] then
                        ini.car.infinitefuel = settings.car.infinitefuel[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Áûñòðàÿ ïîñàäêà â àâòî', settings.car.fastenter) then
                    if settings.cfg.autosave[0] then
                        ini.car.fastenter = settings.car.fastenter[0]
                        save()
                    end
                end
	            if imgui.ToggleButton(u8'Áûñòðûé âûõîä', settings.car.fastexit) then
                    if settings.cfg.autosave[0] then
                        ini.car.fastexit = settings.car.fastexit[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Áûñòðûé òîðìîç + ôëèï', settings.car.fastbrake) then
                    if settings.cfg.autosave[0] then
                        ini.car.fastbrake = settings.car.fastbrake[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Ïðûæîê àâòî (start)', settings.car.jumpcar) then
                    if settings.cfg.autosave[0] then
                        ini.car.jumpcar = settings.car.jumpcar[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Àíòè êàðñêèëë', settings.car.anticarskill) then
                    if settings.cfg.autosave[0] then
                        ini.car.anticarskill = settings.car.anticarskill[0]
                        save()
                    end
                end
                if imgui.Button(u8'Ôëèïíóòü') then
            ...
            elseif subtab_3 == 2 then
                ...
                if imgui.SliderInt(u8'Ðàäèóñ', settings.car.atrradius, 1, 500) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_3 == 3 then
                if imgui.ToggleButton(u8'SpeedHàck', settings.car.speedhack) then
                    if settings.cfg.autosave[0] then
                        ini.car.speedhack = settings.car.speedhack[0]
                        save()
                    end
                end
                if imgui.Button(u8'Íàñòðîéêè ñïèäõàêà') then
                    imgui.OpenPopup(u8'Íàñòðîéêè ñïèäõàêà')
                end
                if imgui.BeginPopupModal(u8'Íàñòðîéêè ñïèäõàêà', _, imgui.WindowFlags.AlwaysAutoResize) then
                    if imgui.SliderFloat(u8'Ñèëà óñêîðåíèÿ', settings.car.slider_mult, 0.001, 100.0) then
			            if settings.cfg.autosave[0] then
			                ini.car.mult = settings.car.slider_mult[0]
			                save()
			            end
		            end
	        	    if imgui.SliderFloat(u8'Ìàêñèìàëüíàÿ ñêîðîñòü', settings.car.slider_limit, 0.01, 1000.0) then
			            if settings.cfg.autosave[0] then
			                ini.car.limit = settings.car.slider_limit[0]
			                save()
			            end
		            end
		            if imgui.SliderFloat(u8'Øàã (ìñ)', settings.car.slider_timestep, 0.0, 1.0) then
			            if settings.cfg.autosave[0] then  
			                ini.car.timestep = settings.car.slider_timestep[0]
			                save()
			            end
		            end
		            if imgui.ToggleButton(u8'Áåçîïàñíàÿ ñêîðîñòü ïîåçäà', settings.car.safe_train_speed) then
			            if settings.cfg.autosave[0] then 
        ...
        elseif tab == 4 then
            ...
            imgui.SameLine()
            if imgui.Button(u8"Íàñòðîéêè ESP") then
                subtab_4 = 2
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_4 == 1 then
                if imgui.ToggleButton(u8'Âêëþ÷èòü ðèñîâêó', settings.ESP.drawing) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.drawing = settings.ESP.drawing[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Êîðîáêè', settings.ESP.enabled_boxes) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_boxes = settings.ESP.enabled_boxes[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Íèêè', settings.ESP.enabled_nicks) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_nicks = settings.ESP.enabled_nicks[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Ñêåëåòû', settings.ESP.enabled_bones) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_bones = settings.ESP.enabled_bones[0]
                        ini.car.speedhack = settings.car.speedhack[0]
                    end
                end
                if imgui.ToggleButton(u8'Ëèíèè', settings.ESP.enabled_lines) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_lines = settings.ESP.enabled_lines[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Çäîðîâüå', settings.ESP.enabled_health) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.enabled_health = settings.ESP.enabled_health[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Áðîíÿ', settings.ESP.enabled_armor) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_4 == 2 then
                if imgui.SliderInt(u8'Ïîçèöèÿ ëèíèè', settings.ESP.lineposition, 0, 1) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.lineposition = settings.ESP.lineposition[0]
                        save()
                    end
                end
                if imgui.SliderInt(u8'Òîëùèíà ëèíèè', settings.ESP.linethinkness, 1, 4) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.linethinkness = settings.ESP.linethinkness[0]
                        save()
                    end
                end
                if imgui.SliderInt(u8'Òîëùèíà ñêåëåòîâ', settings.ESP.skeletonthinkness, 1, 4) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.skeletonthinkness = settings.ESP.skeletonthinkness[0]
                        save()
                    end
                end
                if imgui.SliderInt(u8'Òîëùèíà êîðîáîê', settings.ESP.boxthinkness, 1, 4) then
                    if settings.cfg.autosave[0] then
                        ini.ESP.boxthinkness = settings.ESP.boxthinkness[0]
                        save()
                    end
                end
                if imgui.SliderFloat(u8'Ðàçìåð íèêíåéìîâ', settings.ESP.nicknamesize, 1, 10) then
                    if settings.cfg.autosave[0] then
                ...
                if settings.ESP.enabled_boxes[0] then
                    if imgui.ToggleButton(u8'Ãðàäèåíòíàÿ çàïîëíåííîñòü', settings.ESP.enabled_fillgradient) then
                        if settings.cfg.autosave[0] then
                            ini.ESP.enabled_fillgradient = settings.ESP.enabled_fillgradient[0]
                            save()
                        end
                    end
                    if imgui.SliderFloat(u8'Çàïîëíåííîñòü êîðîáîê', settings.ESP.fillboxvalue, 0.0, 1.0) then
                        if settings.cfg.autosave[0] then
        ...
        elseif tab == 5 then
            ...
            imgui.SameLine()
            if imgui.Button(u8"ÒÏ ê ñòèðàëêàì") then
                subtab_5 = 2
            end
            imgui.SameLine()
            if imgui.Button(u8"Óòèëèòû") then
                subtab_5 = 3
            end
            imgui.SameLine()
            if imgui.Button(u8"Ãðóç÷èê") then
                subtab_5 = 4
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_5 == 1 then
                if imgui.Button(u8'Óäàëèòü êëåòêè') then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                ...
                imgui.SameLine()
                if imgui.Button(u8'Óäàëèòü äâåðü') then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                ...
                imgui.SameLine()
                if imgui.Button(u8'Óäàëèòü çàáîðû') then
                    for _, obj in pairs(getAllObjects()) do
                        local modeid = getObjectModel(obj)
                ...
                if imgui.ToggleButton(u8'Aâòoìàòè÷.', settings.tsr.autormcell) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                if imgui.ToggleButton(u8'Aâòîìàòè÷.', settings.tsr.autormdoors) then
                    if settings.cfg.autosave[0] then
                ...
                imgui.SameLine()
                if imgui.ToggleButton(u8'Àâòoìàòè÷.', settings.tsr.autormfence) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_5 == 2 then
                ...
                if imgui.Button(u8'Ãðÿçíàÿ îäåæäà') then
                    setCharCoordinates(PLAYER_PED, 1396.1380, 1354.9414, 10)
                end
                imgui.SameLine()
                if imgui.Button(u8'×èñòàÿ îäåæäà') then
                    setCharCoordinates(PLAYER_PED, 1397.8023, 1357.9216, 10.9390)
                end
            elseif subtab_5 == 3 then
                if imgui.ToggleButton(u8'Âõ íà ìóñîð', settings.render.musortsr) then
                    if settings.cfg.autosave[0] then
                        ini.render.musortsr = settings.render.musortsr[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Áîò êîðîáêè RAGE', tsrragebot) then
                    botstep = 0
                end
                if imgui.SliderInt(u8'Çàäåðæêà', settings.tsr.tsrbotwait, 100, 5000) then
                    if settings.cfg.autosave[0] then
        ...
        elseif tab == 6 then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button(u8"Êíîïî÷êè") then
                subtab_6 = 1
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_6 == 1 then
                if imgui.Button(fa.COMMENT..u8'  Î÷èñòèòü ÷àò', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    for i = 1, 90 do
                        sampAddChatMessage("", -1)
                    end
                end
                imgui.SameLine()
                if imgui.Button(fa.SKULL_CROSSBONES..u8'  Óìåðåòü', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    setCharHealth(PLAYER_PED, 0)
                end
                imgui.SameLine()
                if imgui.Button(fa.HOUSE..u8'  Ñïàâí', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    sendSpawn()
                end
                imgui.SameLine()
                if imgui.Button(fa.PERSON_WALKING..u8'  Âêë. ïðûæîê', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    lua_thread.create(function()
                ...
                imgui.SameLine()
                if imgui.Button(fa.LOCATION_DOT..u8'  ÒÏ ìåòêà', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    result, x, y, z = getTargetBlipCoordinatesFixed()
                    if result then setCharCoordinates(PLAYER_PED, x, y, z) end
                end
                if imgui.Button(fa.WIFI..u8'  Ðåêîííåêò v2', imgui.ImVec2(125 * MONET_DPI_SCALE, 62 * MONET_DPI_SCALE)) then
                    notf4(u8'Ðåêîííåêòèìñÿ...')
                    local bs = raknetNewBitStream()
                    raknetBitStreamWriteInt8(bs, sf.PACKET_DISCONNECTION_NOTIFICATION)
                    raknetSendBitStreamEx(bs, sf.SYSTEM_PRIORITY, sf.RELIABLE, 0)
        ...
        elseif tab == 7 then
            ...
            imgui.PushStyleColor(imgui.Col.ButtonActive, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            imgui.PushStyleColor(imgui.Col.ButtonHovered, imgui.ImVec4(0.09, 0.09, 0.10, 1.00))
            if imgui.Button(u8"Îòêðûòèå ìåíþ") then
                subtab_7 = 1
            end
            imgui.SameLine()
            ...
            imgui.SameLine()
            if imgui.Button(u8"Ñêðèïò") then
                subtab_7 = 4
            end
            imgui.PopStyleColor(3)
            imgui.Separator()
            if subtab_7 == 1 then
                if imgui.ToggleButton(u8'Ïîëîñêà ñíèçó ýêðàíà', settings.menu.openbutton) then
                    if settings.cfg.autosave[0] then
                        ini.menu.openbutton = settings.menu.openbutton[0]
                        save()
                    end
                end
                if imgui.ToggleButton(u8'Êíîïêà "Menu"', settings.menu.openbutton2) then
                    if settings.cfg.autosave[0] then
                        ini.menu.openbutton2 = settings.menu.openbutton2[0]
                        save()
                    end
                end
		        if imgui.SliderInt(u8'Øèðèíà êíîïêè ìåíþ', settings.menu.slideropenbuttonwidth, 30, 150) then
                    if settings.cfg.autosave[0] then
                        ini.menu.slideropenbuttonwidth = settings.menu.slideropenbuttonwidth[0]
                        save()
                    end
                end
		        if imgui.SliderInt(u8'Âûñîòà êíîïêè ìåíþ', settings.menu.slideropenbuttonheight, 30, 150) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_7 == 2 then
                if imgui.Button(u8'Íàñòðîéêè ìåíþ') then
		            menusettings[0] = not menusettings[0]
		        end
		        if imgui.ToggleButton(u8'Âêëþ÷èòü ñïåöýôôåêòû ìåíþ', settings.menu.draweffects) then
		            if settings.cfg.autosave[0] then
		                ini.menu.draweffects = settings.menu.draweffects[0]
		                save()
		            end
		        end
                if imgui.ToggleButton(u8'Âàòåðìàðêà', settings.menu.watermark) then
                    if settings.cfg.autosave[0] then
            ...
            elseif subtab_7 == 3 then
                if imgui.ToggleButton(u8'Îòîáðàæàòü ïîäñêàçêè', settings.menu.showinfo) then
                    if settings.cfg.autosave[0] then
                        ini.menu.showinfo = settings.menu.showinfo[0]
                        save()
                    end
                end
		        if imgui.Button(u8'Ïîêàçàòü êíîïêó âçàèìîäåéñòâèÿ') then
		        ...
		        imgui.SameLine()
		        if imgui.Button(u8'Êîìàíäû ñêðèïòà') then
		            imgui.OpenPopup(u8'Êîìàíäû ñêðèïòà')
		        end
                if imgui.BeginPopupModal(u8'Êîìàíäû ñêðèïòà', _, imgui.WindowFlags.AlwaysAutoResize) then
                    imgui.Text(u8'/miku - îñíîâíîå ìåíþ ñêðèïòà')
                    imgui.Text(u8'/tpc - òåëåïîðò ìåòêà (äëÿ íóáî ðï, ìîìåíòàëüíûé)')
                    imgui.Text(u8'/jump - âêëþ÷èòü ïðûæîê (áàã, êîãäà óïàë è ïåðñ íå âñòàåò)')
                    imgui.Text(u8'/rc - ðåêîííåêò')
                    imgui.Text(u8'/mikureload - ïåðåçàãðóçêà ñêðèïòà')
                    if imgui.Button(u8' Çàêðûòü') then
                        imgui.CloseCurrentPopup()
                    end
                    imgui.EndPopup()
            ...
            elseif subtab_7 == 4 then
                if imgui.Button(u8'Ïîêàçàòü mimgui demo') then
		            custommimguiStyle[0] = not custommimguiStyle[0]
		        end
                if imgui.Button(u8'Îáíîâèòüñÿ íà ñòàáèëüíóþ âåðñèþ') then
                    updateScript(lmUrl, lmPath)
                end
                if imgui.Button(u8'Îáíîâèòüñÿ íà Áåòà-âåðñèþ') then
                ...
                if imgui.IsItemHovered() then
                    ...
                    imgui.TextUnformatted(u8'Âûêëþ÷èòü ñêðèïò')
                ...
                if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                    imgui.PushTextWrapPos(450)
                    imgui.TextUnformatted(u8'Ïåðåçàãðóçèòü ñêðèïò')
                    imgui.PopTextWrapPos()
                    imgui.EndTooltip()
                end
                imgui.SameLine()
                if imgui.Button(fa.FLOPPY_DISK, imgui.ImVec2(40 * MONET_DPI_SCALE, 40 * MONET_DPI_SCALE)) then
                    imgui.OpenPopup(u8'Ñîõðàíåíèå')
                end
                if imgui.BeginPopupModal(u8'Ñîõðàíåíèå', _, imgui.WindowFlags.NoResize) then
                    local pSize = imgui.ImVec2(300, 500)
                    imgui.SetWindowSizeVec2(imgui.ImVec2(230 * MONET_DPI_SCALE, 150 * MONET_DPI_SCALE))
                    if imgui.Button(u8'Ñîõðàíèòü íàñòðîéêè', imgui.ImVec2(205 * MONET_DPI_SCALE, 40)) then
                        ini.silent.salo = settings.silent.salo[0]
                        ...
                        ini.battery.lowBatteryLevel = settings.battery.lowBatteryLevel[0]
                        save()
                        notf1(u8'Íàñòðîéêè ñîõðàíåíû!')
                        imgui.CloseCurrentPopup()
                    end
                    if imgui.ToggleButton(u8'Àâòîñîõðàíåíèå', settings.cfg.autosave) then
                        ini.cfg.autosave = settings.cfg.autosave[0]
                        save()
                    end
                    if imgui.Button(u8'Çàêðûòü', imgui.ImVec2(205 * MONET_DPI_SCALE, 40)) then
                        imgui.CloseCurrentPopup()
                    end
                    imgui.EndPopup()
                end
                if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                    imgui.PushTextWrapPos(450)
                    imgui.TextUnformatted(u8'Ñîõðàíèòü íàñòðîéêè')
                    imgui.PopTextWrapPos()
                    imgui.EndTooltip()
                end
                imgui.SameLine()
                ...
                if imgui.IsItemHovered() then
                    imgui.BeginTooltip()
                    imgui.PushTextWrapPos(450)
                    imgui.TextUnformatted(u8'Óäàëèòü ñêðèïò')
                    imgui.PopTextWrapPos()
                    imgui.EndTooltip()
                end
                if settings.menu.showinfo[0] then  
                    ...
                    if imgui.IsItemHovered() then
                        imgui.BeginTooltip()
                        imgui.PushTextWrapPos(450)
                        imgui.TextUnformatted(u8'Çàæìèòå êíîïêó ïàëüöåì, ÷òîá ïîñìîòðåòü ïîäñêàçêó')
                        imgui.PopTextWrapPos()
                        imgui.EndTooltip()
                    end
                end
                if deletescript then
                    imgui.Text(u8'Âû óâåðåíû?')
                    if imgui.Button(u8'Äà') then
                        os.remove(thisScript().path)
                        os.remove(getWorkingDirectory().."/config/MikuProject.ini")
                        thisScript():unload()
                    end
                    imgui.SameLine()
                    if imgui.Button(u8'Íåò') then
                        deletescript = not deletescript
                    end
                end
            end
...
-- ===== ÄÎÁÀÂËÅÍÍÛÉ ÕÓÊ ÄËß ÀÍÒÈ-ÊÀÐÑÊÈËËÀ (ÁÅÑÑÌÅÐÒÈÅ ÌÀØÈÍÛ) =====
...
-- /* never update */ --
function main()
    -- ===== ÄÎÁÀÂËßÅÌ ÏÅÐÅÌÅÍÍÓÞ ÄËß ÒÀÉÌÅÐÀ ÂÎÑÑÒÀÍÎÂËÅÍÈß =====
    local lastCarRepairTime = 0
    -- =============================================================
    ...
    check_update()
    antifall()
    jniUtil.Toast(u8"Miku Reborn çàãðóæåí!", jniUtil.ToastFlag.LENGTH_SHORT):show()
        ...
        if settings.battery.notifyLowCharge[0] and success or settings.battery.notifyLowCharge[0] and success2 then
            ...
            if getBatteryLevel <= settings.battery.lowBatteryLevel[0] then
                if not settings.battery.stopMessage then
                    ...
                    jniUtil.Toast(u8"[Miku] Íèçêèé çàðÿä áàòàðåè!!", jniUtil.ToastFlag.LENGTH_SHORT):show()
                end
            else
        ...
        if settings.render.ruda[0] then
            for id = 0, 2048 do
                if sampIs3dTextDefined(id) then
                    ...
                    if text:find("Ìåñòîðîæäåíèå ðåñóðñîâ") then
        ...
        if settings.car.godmode2_enabled[0] then
            if isCharInAnyCar(PLAYER_PED) then
                ...
                if veh and doesVehicleExist(veh) then
                    ...
                    if hp < 990.0 then
                        if os.clock() - lastCarRepairTime > 1.0 then
                            setCarHealth(veh, 999.0)
                            lastCarRepairTime = os.clock()
                        end
                    end
                end
            end
        else
            -- Ïðè îòêëþ÷åíèè îïöèè ñáðàñûâàåì çàùèòû, åñëè îíè áûëè óñòàíîâëåíû ðàíåå (íà âñÿêèé ñëó÷àé)
            if isCharInAnyCar(PLAYER_PED) then
                local veh = storeCarCharIsInNoSave(PLAYER_PED)
                if veh and doesVehicleExist(veh) then
...
function events.onServerMessage(clr, txt) 
	if armorbotstate then
		if string.find(txt, 'Âû ñäåëàëè áðàêîâàííóþ äåòàëü') then
			lua_thread.create(function()
		...
		if string.find(txt, 'îòâåòèë âàì') or string.find(txt, 'òåëåïîðòèðîâàíû') then	
		    notf2(u8'Âàì íàïèñàë àäìèíèñòðàòîð, çàêðóãëÿåìñÿ...')
			lua_thread.create(function()
...
function events.onShowDialog(dlgid, stl, tlt, b1, b2, text)
	if armorbotstate and string.find(text, 'Àäìèíèñòðàòîð') or string.find(text, 'òåëåïîðòèðîâàíû') then
		notf2(u8'Âàì íàïèñàë àäìèíèñòðàòîð, çàêðóãëÿåìñÿ...')
		lua_thread.create(function()
			wait(math.random(1470, 1955))
			point = 0
...
function check_update()
    notf4(u8'Ïðîâåðêà íàëè÷èÿ îáíîâëåíèé...')
    ...
    if response and response ~= currentVersion then
        notf4(u8"Íàéäåíî íîâîå îáíîâëåíèå! Âûâîæó îêíî äëÿ çàãðóçêè...")
        found_update[0] = not found_update[0]
    else
        notf1(u8"Ó âàñ àêòóàëüíàÿ âåðñèÿ ñêðèïòà.")
    end
end
...
function updateScript(scriptUrl, scriptPath)
    ...
    if response and response ~= currentVersion then
        ...
        if success then
            notf1(u8"Ñêðèïò óñïåøíî îáíîâëåí. Ïåðåçàãðóçêà..")
            thisScript():reload()
        else
            notf3(u8"Íåèçâåñòíàÿ îøèáêà, íå óäàëîñü îáíîâèòü ñêðèïò.")
        end
    else
        notf1(u8"Ñêðèïò óæå ÿâëÿåòñÿ ïîñëåäíåé âåðñèåé.")
    end
end
...
imgui.OnFrame(function() return settings.menu.watermark[0] end, function(player)
    ...
    if imgui.Begin("##minet", settings.menu.watermark, imgui.WindowFlags.NoMove + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoInputs + imgui.WindowFlags.NoScrollbar) then
        ...
        imgui.SameLine()
        imgui.SetCursorPosX(imgui.GetCursorPosX() - 8)
        imgui.Text(u8("| "..script_ver.." | @arz_run | "))
        imgui.SameLine()
...
function antifall()
    lua_thread.create(function()
        while settings.ped.nofall[0] do
            wait(0)
...
function getBatteryPercentage()
    ...
    if batteryManager == nil then
        sampAddChatMessage("[Miku] Îøèáêà: íå óäàëîñü ïîëó÷èòü ñåðâèñ BatteryManager", -1)
        sampAddChatMessage("[Miku] Ïðîöåíò çàðÿäà â Watermark íå áóäåò îòîáðàæàòüñÿ êîððåêòíî", -1)
        return -1
    end
    ...
    if ok then
        ...
        sampAddChatMessage("[Miku] Îøèáêà âûçîâà getIntProperty(BATTERY_PROPERTY_CAPACITY): " .. tostring(result), -1)
        percentage = -1
    end
